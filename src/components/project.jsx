...
const Project = () => {
  return (
    <section id="project" className="max-w-[1200px] mx-auto z-10 pt-30 mt-20">
      <h2 className="text-4xl font-bold mb-10 text-center">
        My <span className="text-[#2ef]">Projects</span>
      </h2>
      <div className="flex flex-wrap items-center justify-center gap-6  p-6 place-items-center">
        {projects.map((p, index) => (
          <Link
            key={index}
            to={p.url}
            target="_blank"
            className="reveal relative w-full max-w-[350px] min-w-[250px] h-[250px] rounded-2xl overflow-hidden group card-shadow"
          >
            <img
              src={p.image}
              alt={p.name}
